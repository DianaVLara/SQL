...
# List the VIPs (last and first name) and panels they are presenting, and the type of panel. Only list the VIPs who are presenting. 
...
# List the names of the VIPs, the hotel they are staying and the events they are attending.
...
# List the VIPs (last name) each VIP is providing feedback to.

SELECT attendee.member_id
, attendee.last_name
, reports_to.last_name AS provides_feedback_to
FROM vips attendee
JOIN vips reports_to
ON attendee.provides_feedback_to=reports_to.member_id
;

# List the VIPs who are staying at the Marriot.
...
# Find VIPs staying at the hotel with the least reservations.
